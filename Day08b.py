...
# join all circuits until there is just one
def join_all_circuits(circuits, all_pairs_of_boxes):
    for pair in all_pairs_of_boxes:
        if len(circuits) > 1:
            join_circuits(circuits,pair)
        if len(circuits) == 1:
            print("Part 2 Answer:",pair.box1.x*pair.box2.x)
            join_circuits(circuits,pair)
            return circuits
...
# part 2:
join_all_circuits(circuits, all_pairs_of_boxes)
